						...
						<td class="centerStage" style="vertical-align: top"><table id="wtTable8" style="width: 100%; border-spacing: 0px" cellspacing="0" cellpadding="0" border="0">
							...
							<!--CONTAINER-->							
											...
											<tr>
												<td class="TitleTopic"><span id="ConnectFramework_wt7_block_wt1_wtTABZONE">
													<table class='tabs' cellpadding='0' cellspacing='0'>
														...
														<td  id='TAB5' class='on'  ><a  onclick="javascript:showProcessing('processing');" href='abastecimento.php'>&nbsp;Abastecimento Totais</a></td>
														<? if ($Tipo == 1){ ?>
                                ...
                                <tbody>
                                     ...
                                     <?php
									 $sql ="SELECT a.preco_litro,a.numero_talao,a.contador,a.id,a.ficheiro,a.data,a.data_abastecimento,a.user,a.cliente,a.viatura,s.preco,a.litros,s.valor_inicial as stockInicial
											from abastecimento a
											LEFT join stock s on(s.id=a.stock)
									;
									";
									...
									// $sql="SELECT* from abastecimento";		
                                     ...
                                     $result = $dbo->query($sql);
                                      ...
                                      while($row1 = mysql_fetch_array ($result)){
										$preco_litro = $row1['preco_litro'];
										$ficheiro = ($row1['ficheiro']=="")? $ficheiro="<a href='ficheiros_abastecimento1.php?id=".$row1['id']."'>Não</a>" : $ficheiro="<a href='ficheiros/".$row1['ficheiro']."' target='_blank'><img width='16' height='16' border='0' alt='Ficheiro' title='Ficheiro' src='images/acroread.png'></a>";
							?>
									<tr valign="top" style="height:28px;">
                                               <td style="display:none;"><?=($row1['data'])?></td>
                                               <td><?=data1($row1['data'])?></td>
                                               <td><?=data1($row1['data_abastecimento'])?></td>
                                               <td align="right"><?=mostraJoin($row1['user'],"utilizadores","user_id","username")?></td>
                                               <td align="right"><?=($row1['cliente'])?></td> 
                                               <td align="right"><?=mostraJoin($row1['viatura'],"viatura","id","matricula")?></td>
                                              
											   <td align="right"><?=($row1['litros'])?></td>
											   <td align="right"><?=number_format($preco_litro,3)?>&euro;</td>
											   <td align="right"><?=number_format($row1['litros'] * $preco_litro,2)?>&euro;</td>
											   <td align="right"><?=($row1['contador'])?></td>
											   <td align="right"><?=($row1['numero_talao'])?></td>
											   <td align="right"><?=$ficheiro?></td>
									</tr>	

                                    <?php

											$totaisLitros+= number_format($row1['litros'],2);
											$totais+= ($row1['litros'] * $preco_litro);
									...
									<tfoot>
										<td colspan="6" align="right"><?=number_format($totaisLitros,2)?> Litros</td>
										<td colspan="2" align="right"><?=number_format($totais,2)?>&euro;</td>
									</tfoot>
